...
@section('javascript')
    ...
    <script type="text/javascript">
      $(function () {
        ...
        $('#btn-recalculate-loans').click(function () {
          var $btn = $(this).button('loading');
          $.post("{{ route('loans.recalculate') }}").done(function (response) {
        ...
        $('#btn-recalculate-invests').click(function () {
          var $btn = $(this).button('loading');
          $.post("{{ route('invests.recalculate') }}").done(function (response) {
            alert(response.message);
          }).fail(function (response) {
            alert('{{ trans('fi.error') }}: ' + $.parseJSON(response.responseText).message);
          }).always(function () {
                ...
                <div class="nav-tabs-custom">
                    ...
                    <div class="tab-content">
                        ...
                        <div id="tab-loans" class="tab-pane">
                            @include('settings._loans')
                        </div>
                        <div id="tab-invests" class="tab-pane">
                            @include('settings._invests')
                        </div>
