...
class InvestImporter extends AbstractImporter
{
    public function getFields()
    {
        return [
            'invest_date' => '* ' . trans('fi.date'),
            'company_profile' => '* ' . trans('fi.company_profile'),
            'client_name' => '* ' . trans('fi.client_name'),
            'number' => '* ' . trans('fi.invest_number'),
    ...
    public function importData($input)
    {
        ...
        $handle = fopen(storage_path('invests.csv'), 'r');
        ...
        while (($data = fgetcsv($handle, 1000, ',')) !== false) {
            if ($row !== 1) {
                ...
                if (strtotime($record['invest_date'])) {
                    $record['invest_date'] = date('Y-m-d', strtotime($record['invest_date']));
                }
                ...
                // Assign the invest to the current logged in user
                $record['user_id'] = $userId;

                // The record *should* validate, but just in case...
                if ($this->validateRecord($record)) {
                    Invest::create($record);
                }
            }
            $row++;
        }
