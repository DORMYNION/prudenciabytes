...
class LoanItemImporter extends AbstractImporter
{
    ...
    public function importData($input)
    {
        ...
        $handle = fopen(storage_path('loanItems.csv'), 'r');
        ...
        while (($data = fgetcsv($handle, 1000, ',')) !== false) {
            if ($row !== 1) {
                ...
                $loan = Loan::where('number', $record['loan_id'])->first();
                ...
                if ($loan) {
                    ...
                    if ($this->validateRecord($record)) {
                        ...
                        $loan->items()->create($record);

                        event(new LoanModified($loan));
                    }
                }
            }
