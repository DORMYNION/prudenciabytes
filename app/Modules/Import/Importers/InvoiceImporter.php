...
class LoanImporter extends AbstractImporter
{
    ...
    public function importData($input)
    {
        ...
        $handle = fopen(storage_path('loans.csv'), 'r');
        ...
        while (($data = fgetcsv($handle, 1000, ',')) !== false) {
            if ($row !== 1) {
                ...
                if (strtotime($record['loan_date'])) {
                    $record['loan_date'] = date('Y-m-d', strtotime($record['loan_date']));
                }
                ...
                // Assign the loan to the current logged in user
