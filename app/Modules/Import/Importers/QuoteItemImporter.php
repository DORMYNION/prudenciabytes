...
class InvestItemImporter extends AbstractImporter
{
    public function getFields()
    {
        return [
            'invest_id' => '* ' . trans('fi.invest_number'),
    ...
    public function importData($input)
    {
        ...
        $handle = fopen(storage_path('investItems.csv'), 'r');
        ...
        while (($data = fgetcsv($handle, 1000, ',')) !== false) {
            if ($row !== 1) {
                ...
                $invest = Invest::where('number', $record['invest_id'])->first();

                if ($invest) {
                    $record['invest_id'] = $invest->id;
                    ...
                    if ($this->validateRecord($record)) {
                        ...
                        $invest->items()->create($record);

                        event(new InvestModified($invest));
                    }
                }
            }
