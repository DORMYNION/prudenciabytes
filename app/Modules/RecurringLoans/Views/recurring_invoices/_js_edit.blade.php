<script type="text/javascript">
  ...
  $(function () {
    ...
    $('#btn-copy-recurring-loan').click(function () {
      $('#modal-placeholder').load('{{ route('recurringLoanCopy.create') }}', {
    ...
    $('.btn-delete-recurring-loan-item').click(function () {
      if (!confirm('{!! trans('fi.delete_record_warning') !!}')) return false;
      var id = $(this).data('item-id');
      $.post('{{ route('recurringLoanItem.delete') }}', {
        id: id
      }).done(function () {
        $('#tr-item-' + id).remove();
        $('#div-totals').load('{{ route('recurringLoanEdit.refreshTotals') }}', {
          id: {{ $recurringLoan->id }}
        });
      });
    });

    $('.btn-save-recurring-loan').click(function () {
      var items = [];
      var display_order = 1;
      var custom_fields = {};
      ...
      $('.custom-form-field').each(function () {
        var fieldName = $(this).data('recurring_loans-field-name');
        if (fieldName !== undefined) {
          custom_fields[$(this).data('recurring_loans-field-name')] = $(this).val();
        }
      });

      $.post('{{ route('recurringLoans.update', [$recurringLoan->id]) }}', {
        items: items,
        terms: $('#terms').val(),
        footer: $('#footer').val(),
        currency_code: $('#currency_code').val(),
        ...
        $('#div-recurring-loan-edit').load('{{ route('recurringLoanEdit.refreshEdit', [$recurringLoan->id]) }}', function () {
